...
public class MyBotNoNull : IChessBot
{
    ...
    private int bestMoveScore, bestMoveDepth, nodesChecked;//#DEBUG

    private Move bestRootMove = Move.NullMove;
    ...
    public Move Think(Board _board, Timer _timer)
    {
        ...
        {
            //pvInfo.Clear();
            Search(i, CheckmateValue, -CheckmateValue, 0, 0, true,
                pvInfo//#DEBUG
                );
    ...
    int Search(int depth, int alpha, int beta, int ply, int extensions, Boolean allowNull,
        ...
        )
    {
        ...
        if (allowNull && !board.IsInCheck() && depth >= 3)//#DEBUG
        {//#DEBUG
            board.TrySkipTurn();//#DEBUG
            int score = -Search(depth - 3, beta - 1, 1 - beta, ply, extensions, false,//#DEBUG
                childPV//#DEBUG
                );//#DEBUG
            board.UndoSkipTurn();//#DEBUG
            if (score >= beta) return beta;//#DEBUG
        }//#DEBUG
        ...
        foreach (Move move in moves)
        {
            ...
            board.MakeMove(move);
            int score, newDepth = depth - 1, extension = 0;
            ...
            ply++;
            if (bSearchPv) score = -Search(newDepth, -beta, -alpha, ply, extensions, allowNull
                , childPV//#DEBUG
            );
            else
            {
                if (movesChecked++ > 5 && extension == 0) score = -Search(depth / 2, -alpha - 1, -alpha, ply, extensions, allowNull
                    , childPV//#DEBUG
                );
                else score = -Search(newDepth, -alpha - 1, -alpha, ply, extensions, allowNull
                    , childPV//#DEBUG
                );
                
                if (score > alpha && score < beta) score = -Search(newDepth, -beta, -alpha, ply, extensions, allowNull
                    , childPV//#DEBUG
                    );
            }
    ...
    {
        TTEntry entry = tt[board.ZobristKey % entries];
        if(entry.move == move && entry.key == board.ZobristKey) return 1000;
    ...
    int Evaluation() {
        ...
        Square whiteKingSquare = board.GetKingSquare(true), blackKingSquare = board.GetKingSquare(false);
        int distBetweenKings = Math.Abs(whiteKingSquare.File - blackKingSquare.File) + Math.Abs(whiteKingSquare.Rank - blackKingSquare.Rank);
        //
        return (midGame * phase + endGame * (24 - phase) - distBetweenKings * (24 - phase) * 8) / 24 * (board.IsWhiteToMove ? 1 : -1);
    }
}